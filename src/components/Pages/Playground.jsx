...
// import React, { useState, useEffect, useRef } from 'react';
// import { Link } from 'react-router-dom';
// import { HiOutlinePlay, HiOutlineCloudArrowUp, HiOutlineCommandLine, HiOutlineArrowLeft } from "react-icons/hi2";
// import { toast } from 'sonner'; // Just import toast, not Toaster
...
//     const saveToDatabase = () => {
//         console.log("Saving to MongoDB:", { ...session, timeTaken: seconds });
//         setSession(prev => ({ ...prev, isSaved: true, lastSaved: new Date().toLocaleTimeString() }));
//         toast.success("Progress Saved");
//     };

//     return (
//         <div className="h-screen w-full bg-[#0F172A] text-[#CBD5E1] font-inter flex flex-col overflow-hidden">
//             <nav className="h-16 border-b border-[#334155] bg-[#1E293B]/50 flex items-center justify-between px-8 shrink-0 relative">
//                 <div className="flex items-center gap-6">
//                     <Link to="/" className="text-[#94A3B8] hover:text-white transition-colors"><HiOutlineArrowLeft size={22} /></Link>
//                     <input value={session.title} onChange={(e) => setSession({ ...session, title: e.target.value })} className="bg-transparent text-white font-bold text-lg outline-none" />
//                 </div>
...
//                     <span className={`text-xs font-medium ${session.isSaved ? 'text-green-500' : 'text-red-400'}`}>
//                         {session.isSaved ? `Saved` : 'not saved yet'}
//                     </span>
//                     <button onClick={saveToDatabase} className="flex items-center gap-2 bg-[#1E293B] border border-[#334155] px-4 py-2 rounded-lg hover:bg-[#334155]">
//                         <HiOutlineCloudArrowUp className="text-[#3B82F6] text-xl" /><span>Save</span>
//                     </button>
//                     <button onClick={runCode} disabled={isLoading} className="bg-[#3B82F6] px-6 py-2 rounded-lg font-bold hover:bg-[#2563EB]">{isLoading ? "..." : "Run"}</button>
//                 </div>
//             </nav>

//             <main className="flex flex-grow overflow-hidden">
//                 <section className="w-[65%] flex flex-col border-r border-[#334155]">
//                     <div className="flex items-center justify-between px-6 py-3 bg-[#1E293B]/30 border-b border-[#334155]">
...
//                         <div className="relative group">
//                             <select value={session.language} onChange={(e) => handleLanguageChange(e.target.value)} className="appearance-none bg-[#020617] border border-[#334155] text-[#94A3B8] text-xs font-bold py-1.5 pl-3 pr-8 rounded-md outline-none uppercase">
//                                 <option value="c">C</option>
//                                 <option value="c++">C++</option>
//                                 <option value="python">Python</option>
//                                 <option value="javascript">JavaScript</option>
//                                 <option value="java">Java</option>
//                             </select>
//                             <div className="absolute inset-y-0 right-2 flex items-center pointer-events-none text-[#475569] group-hover:text-[#3B82F6]">
//                                 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-3 h-3"><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
//                             </div>
//                         </div>
//                     </div>
//                     <textarea value={session.code} onChange={handleCodeChange} className="flex-grow w-full bg-[#020617] p-8 font-mono text-sm outline-none resize-none text-[#E2E8F0]" spellCheck="false" />
//                 </section>
//                 <section className="w-[35%] bg-[#020617] flex flex-col">
//                     <div className="px-5 py-3 bg-[#1E293B]/30 border-b border-[#334155] text-[#94A3B8] text-xs font-bold uppercase tracking-widest flex items-center gap-2"><HiOutlineCommandLine size={16} /> Output</div>
//                     <div className="flex-grow p-6 font-mono text-sm overflow-auto text-white">
//                         <pre className={`${session.output.toLowerCase().includes("error") ? 'text-red-400' : 'text-white'} whitespace-pre-wrap`}>
//                             {session.output || "# Click Run to execute code..."}
//                         </pre>
//                     </div>
